...
bool HwCameraDevice::Open(uintptr_t callbackLocalKey,
                          ...
                          bool *outIsNull)
{
    ...
    *outIsNull        = true;
    ...
    HwParcel request;
    /* open() is defined in V3.2 → V3.2 descriptor. */
    request.WriteInterfaceToken(kDescriptorV3_2);
    /*
     * Strong sp<ICameraDeviceCallback> as a local binder.  Kernel
     * translates BINDER_TYPE_BINDER → BINDER_TYPE_HANDLE for the
     * receiver.  Both `binder` and `cookie` get set to
     * callbackLocalKey for trivial server-side dispatch.
     */
    request.WriteFlatBinder(callbackLocalKey);

    HwBinderClient::Reply reply;
    auto rc = client_->Transact(handle_, TRANSACTION_open, request, &reply);
    if (rc != HwBinderClient::Result::Ok) {
        CAMERA_VDI_LOGE("HwCameraDevice::Open: transact rc=%{public}s",
    ...
    if (reader.IsStatus()) {
        int32_t st = 0;
        reader.ReadInt32(&st);
        CAMERA_VDI_LOGE("HwCameraDevice::Open: status_t=%{public}d", st);
        return false;
    }

    int32_t ex = 0;
    if (!reader.ReadInt32(&ex)) {
        CAMERA_VDI_LOGE("Open: reply too short for exception code");
        return false;
    }
    if (ex != 0) {
        CAMERA_VDI_LOGE("Open: HIDL exception %{public}d", ex);
        return false;
    }
    if (!reader.ReadInt32(outHalStatus)) {
        CAMERA_VDI_LOGE("Open: reply too short for HAL Status");
        return false;
    }
    if (*outHalStatus != 0) {
        CAMERA_VDI_LOGE("Open: HAL Status=%{public}d (non-OK)", *outHalStatus);
        return false;
    }
    if (!reader.ReadFlatBinder(outSessionHandle, outIsNull)) {
        CAMERA_VDI_LOGE("Open: failed to read flat_binder_object for "
                        "ICameraDeviceSession");
        return false;
    }
    CAMERA_VDI_LOGI("Open: ICameraDeviceSession handle=%{public}u (isNull=%{public}d)",
                    *outSessionHandle, (int)*outIsNull);
    return true;
}

 ...
 *   https://github.com/sailfishos/droidmedia (master, fetched 2026-05-21)
 * for use through libhybris from OHOS-side musl processes — matches the
 * ABI exported by Halium's /android/system/lib64/libdroidmedia.so.  Sister
 * header droidmediabuffer.h is deliberately *not* vendored: it's the
 * internal C++ struct definition and pulls in Android-only system/window.h
 * + gui/BufferQueue.h.  All we need on the OHOS side is the opaque
 * DroidMediaBuffer* typedef this header already provides.
 */
